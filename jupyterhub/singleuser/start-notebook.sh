...
if [[ ! -z "${JUPYTER_ENABLE_LAB}" ]]; then
  jupyterhub-singleuser --config "$HOME/.jupyter/jupyter_server_config.py" --SingleUserNotebookApp.default_url="/lab"
else
  jupyterhub-singleuser --config "$HOME/.jupyter/jupyter_server_config.py" --SingleUserNotebookApp.default_url="/tree"
fi
